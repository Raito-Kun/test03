/**
 * Deploy bulk-delete + upgraded allocation dialog.
 * Files:
 *   - backend: contact-service + controller + routes (bulk-delete endpoint)
 *   - frontend: data-allocation-dialog (random/manual modes) + contact-list (bulk delete UI)
 */
...
async function main() {
  ...
  await runRemote(conn, `cd ${REPO_REMOTE} && docker compose -f docker-compose.prod.yml build backend 2>&1 | tail -10`);
