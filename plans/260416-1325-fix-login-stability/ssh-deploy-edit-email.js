/**
 * Deploy editable account email — backend (schema/service/controller) +
 * frontend (account-edit dialog).
 */
...
const FILES = [
  ...
  'packages/frontend/src/pages/settings/account-edit-dialog.tsx',
];
...
async function main() {
  ...
  console.log('\n=== recreate ===');
  await runRemote(conn, `cd ${REPO_REMOTE} && docker compose -f docker-compose.prod.yml up -d backend frontend 2>&1 | tail -6`);
